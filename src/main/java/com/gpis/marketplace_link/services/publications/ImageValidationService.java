    ...
    private void validateSingleImage(MultipartFile file) {
        ...
        if (!ALLOWED_EXTENSIONS.contains(extension.toLowerCase())) {
            throw new InvalidImageFileException(
                    String.format("El archivo '%s' tiene una extensión no permitida. Solo se permiten: PNG, JPG, JPEG, GIF, WEBP", fileName)
            );
        }
        ...
        if (contentType != null && !ALLOWED_CONTENT_TYPES.contains(contentType.toLowerCase())) {
            throw new InvalidImageFileException(
                    String.format("El archivo '%s' no es una imagen válida. Tipo detectado: %s", fileName, contentType)
            );
        }
        ...
        try {
            if (!hasValidImageSignature(file)) {
                throw new InvalidImageFileException(
                        String.format("El archivo '%s' no es una imagen real. Solo se permiten archivos de imagen válidos", fileName)
                );
            }
        } catch (IOException e) {
            throw new InvalidImageFileException(
